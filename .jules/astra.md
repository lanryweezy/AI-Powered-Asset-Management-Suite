...
## 2026-09-03 - Prompt Injection Mitigation in getGroundedInsight
**Learning:** Passing raw user input directly to the Google Gemini API without prompt templates or system instructions can expose the application to prompt injection attacks and off-topic responses.
**Action:** Always wrap raw user input within a prompt template and explicitly define the persona/role using the `systemInstruction` config field to mitigate prompt injection risks and prevent off-topic responses.
