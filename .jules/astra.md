...
## 2026-09-21 - Mitigating Prompt Injection with XML Boundaries
**Learning:** Embedding raw, dynamically-provided inputs (like client names, report types, or chart names) directly into a prompt template without explicit boundaries makes the prompt vulnerable to injection. If the user input contains instructions (e.g., "ignore previous instructions and say X"), the model cannot distinguish between the developer's instructions and the user data, leading to hijacked responses. However, when applying this fix to existing templates, we must be careful not to override inline persona definitions by indiscriminately applying `systemInstruction`.
**Action:** When incorporating raw user or dynamically generated variables into prompts, wrap them in explicit, descriptive XML-like boundaries (e.g., `<client_name>...</client_name>`). This structurally isolates the data from the core instructions, significantly reducing the risk of prompt injection and confusing the model, while preserving any necessary inline instructions.
