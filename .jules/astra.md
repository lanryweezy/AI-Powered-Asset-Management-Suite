...
## 2025-02-28 - Mitigating Prompt Injection in Raw User Queries
**Learning:** Directly passing raw user input into an AI model prompt (`contents: query`) without wrapping it in a template or setting a system instruction leaves the application vulnerable to prompt injection and off-topic responses, breaking the intended persona.
**Action:** When constructing AI prompts with raw user input for the Google Gemini API, always wrap the input within a prompt template and explicitly define the persona/role using the `systemInstruction` config field to mitigate prompt injection risks and prevent off-topic responses.
