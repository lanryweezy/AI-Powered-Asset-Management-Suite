...
## 2026-09-13 - Mitigating Prompt Injection in Report Summary\n**Learning:** Passing raw user input directly as the `contents` of an AI API call leaves the system vulnerable to prompt injection, where the user can command the model to ignore its purpose and perform unauthorized actions or output off-topic responses.\n**Action:** When constructing AI prompts with raw user input, always wrap the input within a prompt template (e.g., using XML-like tags like `<client_name>`) and explicitly define the persona/role using the `systemInstruction` config field to enforce boundaries and prevent off-topic responses.
