...
## 2025-02-28 - Prompt Injection Mitigation in Gemini API
**Learning:** Passing raw user input directly to the `contents` field of a Gemini API call without a prompt template or system instruction poses a severe prompt injection vulnerability. Users can supply inputs that bypass intended boundaries or cause the AI to generate off-topic, harmful, or irrelevant content.
**Action:** Always wrap user input within a structured prompt template and explicitly define the AI's role and boundaries using the `systemInstruction` field in the config object to restrict the scope of its answers and prevent malicious instructions.
