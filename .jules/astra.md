...
## 2025-02-28 - Mitigating Prompt Injection with systemInstruction
**Learning:** When constructing AI prompts with raw user input (like names or report types), embedding the persona in the main prompt string leaves the system vulnerable to prompt injection, where user input overrides the intended behavior.
**Action:** Always wrap the input within a prompt template and explicitly define the persona/role using the `systemInstruction` config field to mitigate prompt injection risks and prevent off-topic responses.
