...
// AI Quality Insight: Define a shared, explicit system prompt for the FinAI persona
export const FINAI_SYSTEM_PROMPT = 'You are a helpful AI assistant for a financial asset management platform focused on the Nigerian market. Your name is "FinAI". Be concise and professional. You can answer questions about portfolio data, market trends, and general financial concepts related to Nigeria.';
...
export const getPortfolioOptimization = async (assets: PortfolioAsset[]): Promise<OptimizationSuggestion[]> => {
    ...
    try {
        ...
        // AI Quality Insight: Validate that every item in the array has the required properties
...
export const generateReportSummary = async (clientName: string, reportType: string, reportingPeriod: string, assets: PortfolioAsset[]): Promise<string> => {
    ...
    // AI Quality Insight: Wrap raw user input in a prompt template and explicitly set systemInstruction to mitigate prompt injection.
    const prompt = `You are a professional financial advisor writing a brief, encouraging summary for a client report based on the provided portfolio data.
    Keep the summary to 3-4 sentences. Use markdown for bolding key terms.

    <client_info>
        <name>${clientName}</name>
        <report_type>${reportType}</report_type>
        <reporting_period>${reportingPeriod}</reporting_period>
    </client_info>
...
export const getGroundedInsight = async (query: string): Promise<GroundedInsight> => {
    ...
    // AI Quality Insight: Wrap raw user input in a prompt template and explicitly set systemInstruction to mitigate prompt injection and enforce persona limits.
    const prompt = `Please provide a grounded insight based on the following user query:\n\n<user_query>\n${query}\n</user_query>`;
