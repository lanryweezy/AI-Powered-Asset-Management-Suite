...
export const generateReportSummary = async (clientName: string, reportType: string, reportingPeriod: string, assets: PortfolioAsset[]): Promise<string> => {
    ...
    const prompt = `Your client's name is ${clientName}.
    ...
    try {
        ...
        const response = await withTimeout(ai.models.generateContent({
            model: "gemini-2.5-flash",
            contents: prompt,
            // AI Quality Insight: Use systemInstruction to mitigate prompt injection risks from raw user input
            config: {
                systemInstruction: "You are a professional financial advisor for a Nigerian asset management firm.",
            }
        }), 10000);
        return response.text;
    } catch (error) {
