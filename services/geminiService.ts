...
export const generateReportSummary = async (clientName: string, reportType: string, reportingPeriod: string, assets: PortfolioAsset[]): Promise<string> => {
    ...
    const prompt = `Please write a brief, professional, and encouraging summary for the client based on their portfolio.
    ...
    <portfolio_assets>${JSON.stringify(assets.map(a => ({ ticker: a.ticker, value: a.value, change: a.change, sector: a.sector })))}</portfolio_assets>`;
